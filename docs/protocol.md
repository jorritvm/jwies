...
`packages/jwies-server/jwies_server/protocol.py`, maar **geen enkele client
importeert die modellen**: zowel de PyQt-client als de browserclient bouwen hun
JSON met de hand op en lezen ze met de hand terug. Dat is bewust - het houdt de
browserclient vrij van een buildstap en de desktopclient vrij van elke
afhankelijkheid op de server. Het gevolg is dat de *waarden* hieronder
vastliggen en de klassenamen niet.
