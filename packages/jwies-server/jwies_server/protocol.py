...
behind, and ``docs/protocol.md`` is the human-readable version.
...
# They used to be a second set of declarations kept honest by a parity test; the
# alias *is* the guarantee now, and the drift it was guarding against cannot
# happen. The names are kept because they read better at the wire boundary.
SuitCode = Suit
BidTypeCode = BidType
...
class ErrorCode(StrEnum):
    ...
    # "het is niet jouw beurt". A second code for the same answer was never
    # constructed.
...
# frames and orders messages, so an envelope only has to carry the version and
# a sequence number.
