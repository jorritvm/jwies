...
fourth ace play together, and the fourth ace's suit is trump.

Ported from ``Table.check_for_trull`` and ``Table.add_default_trull_player_bids``,
with one behavioural fix. The old loop assigned the partner by overwriting a
variable while iterating the seats, so with several single-ace holders the
*last one iterated* won - an arbitrary result flagged in TODO.md. Here the
partner is the first single-ace holder clockwise from the troel player, which
is well-defined and matches how the table is read in practice.
"""
