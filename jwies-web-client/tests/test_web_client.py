...
path the JS fetches, and the client staying a thin, buildless, dependency-free
file server.

The card-id parity checks against the rules engine (does the JS mapping table
still agree with ``jwies_core``?) live in the separate ``tests``
project instead: this project has no dependency on ``jwies_core`` at all, and
...
def test_both_clients_ship_the_same_card_sheet() -> None:
    ...
    the moment the two copies drift apart. It only needs a filesystem read of
    the sibling checkout, not a dependency on jwies-qt-client.
    """
    qt = (
        ROOT.parent
        / "jwies-qt-client"
