...
This is the replacement for ``Table.divide_teams`` - 125 lines of near-identical
if/elif branches, each repeating the same four steps with a different string.
Here the branches are a lookup in ``CONTRACT_CATALOG`` and the differences are
data, so adding a contract needs no code change.
"""
...
def resolve_contract(
    ...
    *,
    ...
    # with the next dealer - the case the old check_if_we_need_to_redeal
    # handled as 'one player asked, nobody joined, player did not go alone'.
