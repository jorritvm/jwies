...
These numbers are tuned by eye; there is no reason to re-derive them.
...
def direction_of(seat: int, your_seat: int | None) -> str:
    ...
    The server sends absolute seats plus ``your_seat``; each client works out
    its own point of view from there.
    """
